...
func warn(h *host, quiet bool) {
	...
	for i := 0; i < pingCount; i++ {
		...
		if !jitter && i > 0 && h.LastRTTs[i] != 0 && minms(h.LastRTTs[i-1])-ms > jitterThreshold && minms(h.LastRTTs[i-1]) > ms*2 {
			jc++
			if jc > 1 {
...
func main() {
	...
	flag.IntVar(&historyLength, "history-length", 60, "number of rounds of pings to keep")
	flag.Int64Var(&jitterThreshold, "jitter-threshold", 50, "alert on jitter over this in ms")
